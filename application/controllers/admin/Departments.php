...
class Departments extends AdminController
{
    ...
    {
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('subdepartments');
        }
        $data['email_exist_as_staff'] = $this->email_exist_as_staff();
        $data['title']                = _l('subdepartments');
         $data['staff'] = $this->staff_model->get('', ['active' => 1]);
         $data['departments'] = $this->visitors_model->get_departments();
        $this->load->view('admin/subdepartments/manage', $data);
    }
    ...
    public function department($id = '')
    {
        ...
        /* Edit or add new sub department */
    public function subdepartment($id = '')
    {
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                $id = $this->departments_model->addsubdepartment($data);
                if ($id) {
                    $success = true;
                    $message = _l('added_successfully', _l('subdepartment'));
                }
                echo json_encode([
                    'success'              => $success,
                    'message'              => $message,
