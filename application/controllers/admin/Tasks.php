...
class Tasks extends AdminController
{
    ...
    public function check()
    {
        ...
        $limo = $this->tasks_model->is_staff_supervisor(get_staff_user_id());
        if($limo){
            echo "true";
        }else{
            echo "false";
    ...
    public function table_reports()
    {
        $this->app->get_table_data('tasks_reports');
    }
    ...
    public function detailed_overview()
    {
       ...
       if ($this->input->post('dept')) {
        ...
        for ($m = 1; $m <= 12; $m++) {
            ...
            $this->db->join(db_prefix().'staff_departments', db_prefix().'task_assigned.staffid='.db_prefix().'staff_departments.staffid','left');
            if(is_numeric($dept_id)){

            $this->db->where(db_prefix().'staff_departments.departmentid', $dept_id);
