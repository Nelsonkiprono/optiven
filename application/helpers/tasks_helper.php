...
function format_members_by_ids_and_names($ids, $names, $hidden_export_table = true, $image_class = 'staff-profile-image-small')
{
    ...
    foreach ($assignees as $key => $assigned) {
        ...
        if ($assigned != '') {
            if($assignee_id == get_staff_user_id()){
                $outputAssignees .= '<a href="' . admin_url('profile/' . $assignee_id) . '" style="color:#ff6f00">ME</a> | ';
    ...
    if ($exportAssignees != '') {
        // $outputAssignees .= '<span class="s">' . mb_substr($exportAssignees, 0, -2) . '</span>';
    }
...
if(isset($id)){
    $CI = & get_instance();
    $CI->db->select('name');
    $CI->db->join(db_prefix().'staff_departments', db_prefix().'departments.departmentid='.db_prefix().'staff_departments.departmentid','left');
    $CI->db->where(db_prefix().'staff_departments.staffid', $id);
    $task = $CI->db->get(db_prefix() . 'departments')->row();
    if ($task) {
        return $task->name;
    }
}
