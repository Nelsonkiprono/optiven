...
class Authentication_model extends App_Model
{
    ...
    public function marketer_login($email, $password)
    {
        if ((!empty($email)) and (!empty($password))) {
            $table = db_prefix() . 'staff';
            $_id   = 'staffid';
            ...
            if ($user) {
                if (!app_hasher()->CheckPassword($password, $user->password)) {
                    return array("success"=>2, "message"=> 'Failed Login Attempt [Email: ' . $email . ', Is Staff Member: Yes,]');
                }
            } else {
                return array("success"=>3, "message"=> 'Non Existing User Tried to Login [Email: ' . $email . ', Is Staff Member: Yes,]');
            }

            if ($user->active == 0) {
                return array("success"=>4, "message"=>'Inactive User Tried to Login [Email: ' . $email . ', Is Staff Member: Yes,]');
            }

            return array("success"=>1, "message"=> 'Login successful', "data"=>$user);
        }

        return array("success"=>5, "message"=> 'An error occured please try again later');
    }
