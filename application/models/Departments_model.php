...
class Departments_model extends App_Model
{
    ...
    public function addsubdepartment($data)
    {
        ...
        $data = hooks()->apply_filters('before_department_added', $data);
        $this->db->insert(db_prefix() . 'subdepartments', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            hooks()->do_action('after_department_added', $insert_id);
            log_activity('New Department Added [' . $data['name'] . ', ID: ' . $insert_id . ']');
     ...
     * @param  boolean (optional)
     * @return mixed
     * Get sub departments where staff belongs
     * If $onlyids passed return only departmentsID (simple array) if not returns array of all sub departments
     */
    public function get_staff_subdepartments($userid = false, $onlyids = false)
    {
        if ($userid == false) {
            $userid = get_staff_user_id();
        }
        if ($onlyids == false) {
            ...
            $this->db->select(db_prefix() . 'staff_subdepartments.subdepartmentid');
        }
        ...
        if ($onlyids == true) {
            $departmentsid = [];
            foreach ($departments as $department) {
                array_push($departmentsid, $department['subdepartmentid']);
            }
