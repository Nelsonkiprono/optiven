...
class Staff_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        if ($staffid) {
            ...
            if (isset($subdepartments)) {
                foreach ($subdepartments as $subdepartment) {
                    $this->db->insert(db_prefix() . 'staff_subdepartments', [
                        'staffid'      => $staffid,
    ...
    public function update($data, $id)
    {
        ...
        $staff_subdepartments = $this->departments_model->get_staff_subdepartments($id);
        if (sizeof($staff_subdepartments) > 0) {
            if (!isset($data['subdepartments'])) {
                $this->db->where('staffid', $id);
                $this->db->delete(db_prefix() . 'staff_subdepartments');
            } else {
                foreach ($staff_subdepartments as $staff_subdepartment) {
                    if (isset($subdepartments)) {
                        if (!in_array($staff_subdepartment['subdepartmentid'], $subdepartments)) {
                            $this->db->where('staffid', $id);
                            $this->db->where('subdepartmentid', $staff_subdepartment['subdepartmentid']);
                            $this->db->delete(db_prefix() . 'staff_subdepartments');
                            if ($this->db->affected_rows() > 0) {
                                $affectedRows++;
                            }
            ...
            if (isset($subdepartments)) {
                foreach ($subdepartments as $subdepartment) {
                    $this->db->where('staffid', $id);
                    $this->db->where('subdepartmentid', $subdepartment);
                    $_exists = $this->db->get(db_prefix() . 'staff_subdepartments')->row();
                    if (!$_exists) {
                        $this->db->insert(db_prefix() . 'staff_subdepartments', [
                            'staffid'      => $id,
                            'subdepartmentid' => $subdepartment,
    ...
    public function setHod($data,$id){
        //tbldepartment_heads
          $hoddata = [
               'hod' => 1,
             ];
        $this->db->where('staffid', $id);
        $this->db->update('staff', $hoddata);
         if ($this->db->affected_rows() > 0) {
    ...
    public function deleteHod($data,$id){
        // tbldepartment_heads
