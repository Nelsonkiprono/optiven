...
class Tasks_model extends App_Model
{
    ...
    public function get_staff_department($id = '')
    {
            $this->db->select('*');

        if (is_numeric($id) && isset($id)) {
            $this->db->limit(1);
            $this->db->where(db_prefix() . 'departments.hod', $id);
            $hod = $this->db->get(db_prefix() . 'departments')->row();
    ...
    // Get count for sub departments in a department
    public function get_subdepartmentsfordepartment($id = '')
    {
        ...
        if (is_numeric($id) && isset($id)) {
            ...
            $hod = $this->db->get(db_prefix() . 'subdepartments');
    ...
    // Get the staff in a department

    public function get_department_staff($dept_id = '')
    {
        ...
        $staffs = $this->db->get()->result_array();
            ...
            foreach ($staffs as $staff) {
                array_push($departmentsid, $staff['staffid']);
            }
        ...
        // Get the staff in a subdepartment

    public function get_sub_department_staff($dept_id = '')
    {
       
        $this->db->select(db_prefix() . 'staff_subdepartments.staffid');
        
        $this->db->from(db_prefix() . 'staff_subdepartments');
        
        $this->db->where('subdepartmentid', $dept_id);
        $staffs = $this->db->get()->result_array();
    ...
    public function get_subdepartment_id($id = ''){
        $this->db->select('*');

        if (is_numeric($id)) {
            $this->db->where(db_prefix() . 'subdepartments.supervisor', $id);
            $client = $this->db->get(db_prefix() . 'subdepartments')->row();

            return $client->subdepartmentid;
        }


        return $this->db->get(db_prefix() . 'subdepartments')->result_array();
    }
    //  Get the hod ids

    public function get_hod_ids(){
         $this->db->select(db_prefix() . 'departments.hod');
        ...
        $staffs = $this->db->get()->result_array();
            ...
            foreach ($staffs as $staff) {
                if (isset($staff['hod'])) {
                    array_push($departmentsid, $staff['hod']);
                } 
            }
    ...
    public function get_adso_ids(){
        ...
        $staffs = $this->db->get()->result_array();
            ...
            $departmentsid = [];
            foreach ($staffs as $staff) {
               if (has_permission('adso', $staff['staffid'], 'yes')) {
                    array_push($departmentsid, $staff['staffid']);
                } 
            }
    ...
    // Get supervisor ids in department
    public function get_supervisor_ids($id = ''){
        $this->db->select(db_prefix() . 'subdepartments.supervisor');
        $this->db->where(db_prefix() . 'subdepartments.departmentid', $id);
        ...
        $staffs = $this->db->get()->result_array();
            ...
            $departmentsid = [];
            foreach ($staffs as $staff) {
                if (isset($staff['supervisor'])) {
                    array_push($departmentsid, $staff['supervisor']);
                } 
            }
    ...
    {
        // #989898 Default not started color
        // #03A9F4 Default not started color
    ...
    // $CI = & get_instance();
        ...
        return $this->db->count_all_results(db_prefix() . 'departments') > 0 ? true : false;
    }
    ...
    public function do_kanban_query($status, $search = '', $page = 1, $count = false, $where = [])
    {
        ...
        */
        ...
        $tasks_where = '(' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid = ' . get_staff_user_id() . '))';
        $tasks_or_where = '';
        ...
        if (is_hod(get_staff_user_id())) {
              $department = $this->get_staff_department(get_staff_user_id());
              $deptId = $department->departmentid;
              //Get the HOD sub departments .
              $count1 = $this->get_subdepartmentsfordepartment($deptId);
              if ($count1>0) {
                 $supervisorids = $this->get_supervisor_ids($deptId);
                $total = count($supervisorids);
                if ($total>0) {

                     $tasks_or_where.=' (' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid IN (' . implode(', ', $supervisorids) . ')))';

                }
                 
              }else { // get tasks for all the department employees
                  $mystaff = $this->get_department_staff($deptId);
                  $countmystaff = count($mystaff);
                  if ($countmystaff>0) {

                     $tasks_or_where.=' (' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid IN (' . implode(', ', $mystaff) . ')))';
        ...
        if (is_supervisor(get_staff_user_id())) {
            $mysupdept = $this->get_subdepartment_id(get_staff_user_id());


            if (isset($mysupdept)) {
                $mydeptstaff = $this->get_sub_department_staff($mysupdept);
                $count2 = count($mydeptstaff);
                if ($count2>0) {
                    $tasks_or_where.= ' (' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid IN (' . implode(', ', $mydeptstaff) . ')))';
                }
            }
         }

         if (has_permission('adso', '', 'yes')) {
                $hodids = $this->get_hod_ids();
                if(count($hodids)>0){
                    $tasks_or_where.= ' (' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid IN (' . implode(', ', $hodids) . ')))';
                 }
        }

        if (has_permission('dso', '', 'yes')) {
                $hodids = $this->get_adso_ids();
                if(count($hodids)>0){
                    $tasks_or_where.= ' (' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid IN (' . implode(', ', $hodids) . ')))';
                 }
         }
        ...
        if ($tasks_or_where != '') {
         $this->db->or_where($tasks_or_where);
       }
    ...
    public function add($data, $clientRequest = false)
    {
        ...
        if ($insert_id) {
            ...
            $this->_send_task_responsible_users_notification($description, 9, 10, 'task_new_comment_to_staff', $additional_data, $insert_id);
            handle_tags_save($tags, $insert_id, 'task');
    ...
    public function is_task_assignee($userid, $taskid)
    {
        ...
        /**
     * Check is user is task supervisor
     * @param  mixed  $userid staff id
     * @param  mixed  $taskid taskid
     * @return boolean
     */
    public function is_task_supervisor($staffid)
    {
        return ($this->is_staff_supervisor($staffid)
        || $this->is_staff_hod($staffid)
        || $this->is_staff_adso($staffid)
        || $this->is_staff_dso($staffid));
    }

    public function is_staff_supervisor($staffid)
    {
        $this->db->select('subdepartmentid');
        $this->db->where('staffid', $staffid);
        $subdepartment = $this->db->get(db_prefix() . 'staff_subdepartments`')->row()->subdepartmentid;
        if (isset($subdepartment)) {
            $this->db->select('supervisor');
            $this->db->where('supervisor', $subdepartment);
            $staff = $this->db->get(db_prefix() . 'subdepartments')->row()->supervisor;
            if (isset($staff)) {
                return true;
            }
        }
    ...
    public function is_staff_hod($staffid)
    {
        $this->db->select('departmentid');
        $this->db->where('staffid', $staffid);
        $departmentid = $this->db->get(db_prefix() . 'staff_departments`')->row()->departmentid;
        if (isset($departmentid)) {
            $this->db->select('hod');
            $this->db->where('hod', $departmentid);
            $staff = $this->db->get(db_prefix() . 'departments')->row()->hod;
            if (isset($staff)) {
                return true;
            }
        }
    ...
    public function is_staff_adso($staffid)
    {
         if (has_permission('adso', $staffid, 'yes')) {
            return true;
         }
         return false;
    }

    public function is_staff_dso($staffid)
    {
         if (has_permission('dso', $staffid, 'yes')) {
            return true;
         }
         return false;
    }
    ...
    private function should_staff_receive_notification($staffid, $taskid)
    {
        ...
        //         || $this->is_task_supervisor($staffid)
        //     );
