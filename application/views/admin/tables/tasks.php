...
$hasPermissionApprove = has_permission('tasks', '', 'approve');
...
$aColumns = [
    ...
    'startdate',
    'duedate',
     get_sql_select_task_asignees_full_names() . ' as assignees',//get_sql_select_task_asignees_full_names() . ' as approver',
...
$current_user = $GLOBALS['current_user']->staffid;
...
foreach ($rResult as $aRow) {
    ...
    if ($hasPermissionDelete && $current_user==get_staff_user_id()) {
    ...
    if ($canChangeStatus) {
        ...
        foreach ($task_statuses as $taskChangeStatus) {
            if ($taskChangeStatus['id']==6) {
               if ($aRow['status'] != $taskChangeStatus['id']) {
                 if (has_permission('tasks', '', 'approve') || is_hod(get_staff_user_id()) || is_supervisor(get_staff_user_id()) && !$aRow['is_assigned'] || has_permission('dso',get_staff_user_id() , 'yes') || has_permission('adso',get_staff_user_id() , 'yes')) {
                    $outputStatus .= '<li>
    ...
    $row[] = format_members_by_ids_and_names($aRow['assigner_ids'], $aRow['assigners']);
