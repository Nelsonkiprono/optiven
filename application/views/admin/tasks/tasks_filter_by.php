...
<div class="_hidden_inputs _filters _tasks_filters">
    ...
    $tasks_filter_departments = $this->visitors_model->get_departments();
    ...
    echo form_hidden('filter_by_date');
    ...
    if(has_permission('tasks','','view')){
        foreach($tasks_filter_departments as $tf_department){
            echo form_hidden('task_department_'.$tf_department['departmentid']);
        }
    }
...
<div class="btn-group pull-right mleft4 mbot25 btn-with-tooltip-group _filter_data" data-toggle="tooltip" data-title="<?php echo _l('filter_by'); ?>">
    <?php if (has_permission('adso', '', 'yes')) { ?>
    <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        By Departments
   </button>
    <?php }?>
   <ul class="dropdown-menu width300">
    ...
    <li class="divider"></li>
    <?php foreach($tasks_filter_departments as $tf_de){ ?>
    <li>
        <a href="#" data-cview="task_department_<?php echo $tf_de['departmentid']; ?>" onclick="dt_custom_view('task_department_<?php echo $tf_de['departmentid']; ?>','<?php echo $view_table_name; ?>','task_department_<?php echo $tf_de['departmentid']; ?>'); return false;">
            <?php echo $tf_de['name']; ?>
        </a>
    </li>
    <?php } ?>
