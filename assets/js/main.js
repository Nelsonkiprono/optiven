...
function _unit_init_data(data, id) {
    ...
    if (id !== '' && typeof(id) != 'undefined') {
        ...
        leadAttachmentsDropzone = new Dropzone("#lead-attachment-upload", appCreateDropzoneOptions({
            ...
            success: function(file, response) {
                response = JSON.parse(response);
                if (this.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
                    _unit_init_data(response, response.id);
                }
            }
        }));
...
function init_unit_modal_data(id, url, isEdit) {

    var requestURL = (typeof(url) != 'undefined' ? url : 'units/unit/') + (typeof(id) != 'undefined' ? id : '');
...
function edit_call_detail(task_id) {
    requestGet('tasks/task/' + task_id).done(function(response) {
        $('#_task').html(response);
        $('#task-modal').modal('hide');
        $("body").find('#_task_modal').modal({ show: true, backdrop: 'static' });
...
// Init call modal and get data from server
function init_call_modal(task_id, comment_id) {
